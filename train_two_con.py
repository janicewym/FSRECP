from fewshot_re_kit.data_loader1 import get_loader, get_loader_pair, get_prompt_loader, get_kgprompt_loader, get_kgtypeprompt_loader
from fewshot_re_kit.framework11 import FewShotREFramework
from fewshot_re_kit.sentence_encoder1 import CNNSentenceEncoder, BERTSentenceEncoder, BERTPAIRSentenceEncoder, RobertaSentenceEncoder, RobertaPAIRSentenceEncoder, \
                                            PromptSentenceEncoder, KGPromptSentenceEncoder, KGTypePromptSentenceEncoder
import models
from models.proto import Proto
from models.my_kgproto_con2_v2_2 import MYKGTProto
from models.bigproto import BigProto
import sys
import torch
...
def main():
    ...
    if encoder_name == 'cnn':
        ...
        sentence_encoder = PromptSentenceEncoder(
                    ...
                    cat_entity_rep=opt.cat_entity_rep,
                    mask_entity=opt.mask_entity)
    elif encoder_name == 'kgprompt':
        pretrain_ckpt = opt.pretrain_ckpt or 'bert-base-uncased'
        sentence_encoder = KGPromptSentenceEncoder(
                    pretrain_ckpt,
                    max_length,
                    cat_entity_rep=opt.cat_entity_rep,
                    mask_entity=opt.mask_entity)
    ...
    if opt.pair:
    ...
    elif opt.prompt and opt.kg and opt.type is False:
        print("Loading entity dict...")
        wikilist = [one.strip("\n") for one in open('./data/wiki.txt')]
        entity2id = dict([(wiki, k+1) for k, wiki in enumerate(wikilist)])
        id2entity = dict([(k+1, wiki) for k, wiki in enumerate(wikilist)])
        print("Loading relation dict...")
        rellist = [rel.strip("\n") for rel in open('./data/relations.txt')]
        rel2id = dict([(rel, k) for k, rel in enumerate(rellist)]) # id 对应行下标为id的embedding
        id2rel = dict([(k, rel) for k, rel in enumerate(rellist)])
        
        train_data_loader = get_kgprompt_loader(opt.train, opt.pid2name, entity2id, rel2id, sentence_encoder,
                N=trainN, K=K, Q=Q, batch_size=batch_size, ispubmed=opt.ispubmed, root=opt.root)
        val_data_loader = get_kgprompt_loader(opt.val, opt.pid2name, entity2id, rel2id, sentence_encoder,
                N=N, K=K, Q=Q, batch_size=batch_size, ispubmed=opt.ispubmed, root=opt.root)
        test_data_loader = get_kgprompt_loader(opt.test, opt.pid2name, entity2id, rel2id, sentence_encoder,
                N=N, K=K, Q=Q, batch_size=batch_size, ispubmed=opt.ispubmed, root=opt.root)
        ...
        entity2type = json.load(open('./data/prior/wiki_type_wiki80.json'))
        type_max_num, types_set = get_type_max_num(entity2type)
        
        # print("Creating dict: EntityType2Id...")
        # type2id = dict([(t, k) for k, t in enumerate(types_set)])
        # id2type = dict([(k, t) for k, t in enumerate(types_set)])
    ...
    if opt.kg and opt.type is False:
        print("Loading relation-weight dictionary...")
        score_head = json.load(open ('./data/prior/score/score_head_relation_dict_wiki80.json'))
        score_tail = json.load(open ('./data/prior/score/score_tail_relation_dict_wiki80.json'))
    
    if model_name == 'proto':
        # model = Proto(sentence_encoder, dot=opt.dot)
        # model = MYProto(sentence_encoder, dot=opt.dot)
        # model = MYKGProto(sentence_encoder, id2entity, id2rel, score_head, score_tail, dot=opt.dot)
        model = MYKGTProto(sentence_encoder, id2entity, id2rel, dot=opt.dot)
    elif model_name == 'bigproto':
        model = BigProto(sentence_encoder, dot=opt.dot)
    else:
        raise NotImplementedError
